...
@interface CreatNewAddreesController () <UIPickerViewDataSource,UIPickerViewDelegate>
...
@implementation CreatNewAddreesController
...
-(void)saveBtnClick{
    if ([self.isDefaulted integerValue]||self.isFromOrder) {
    ...
    if (self.nameTF.text.length&&self.phoneTF.text.length&&self.postalTF.text.length&&self.provinceField.text.length&&self.detailTV.text.length) {
...
-(void)startNetworking{
    if ([ATUtility isConnectionAvailable]) {
        ...
        [self mineCreataddressHttpRequestWithDictionary:dic];
    }
    else{
...
/**
 提交新地址网络请求
 */
-(void)mineCreataddressHttpRequestWithDictionary:(NSDictionary *)dic{
    [ATUtility showMBProgress:self.view];
    [RequestManager mineCreataddressWithParametersDic:dic success:^(NSDictionary *result) {
